...
def load_file(f, label, letter, path_to_training_data, data, data_labels):
...
def load_null_sample(f, letter, path_to_training_data, data, data_labels):
	if not f.endswith(".txt"):
		return
	fs = f.split("-") # split file name to get starting frame
	start = int(fs[1].split('.')[0])//10
    ...
    # if start > 200, then 0 - 100, 50 - 200
...
def load_dataset(path_to_training_data, data, data_labels):
    l_before = len(data)
    print("loading", path_to_training_data)
    for f in sorted(os.listdir(path_to_training_data + "b")):
        load_file(f, 0, "b", path_to_training_data, data, data_labels)
    
    for f in sorted(os.listdir(path_to_training_data + "d")):
        load_file(f, 1, "d", path_to_training_data, data, data_labels)
       
    for f in sorted(os.listdir(path_to_training_data + "g")):
        load_file(f, 2, "g", path_to_training_data, data, data_labels)

    length = len(data) - l_before

    for f in sorted(os.listdir(path_to_training_data + "b")):
        load_null_sample(f, "b", path_to_training_data, data, data_labels)

    for f in sorted(os.listdir(path_to_training_data + "d")):
        load_null_sample(f, "d", path_to_training_data, data, data_labels)

    for f in sorted(os.listdir(path_to_training_data + "g")):
        load_null_sample(f, "g", path_to_training_data, data, data_labels)
